  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      {quote.status === 'rejected' && (
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-300 p-4 rounded-xl flex items-center space-x-3">
        ...
        <CardContent className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-6 gap-4">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-zinc-100">{quote.quote_number}</h1>
                <Badge
                  variant={
                    quote.status === 'accepted'
                      ? 'success'
            ...
            <div className="sm:text-right">
              <span className="text-xs text-slate-400 dark:text-zinc-400 uppercase tracking-wider font-semibold block">Total Amount</span>
              <span className="text-3xl font-black text-slate-900 dark:text-zinc-100">{formatCurrency(quote.total_cents)}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-zinc-800/60 p-4 rounded-lg border border-slate-200 dark:border-zinc-700/80">
            ...
            <div className="font-bold text-slate-900 dark:text-zinc-100 text-base">
              ...
              {quote.customer?.company_name && (
                <span className="font-normal text-slate-500 dark:text-zinc-400 text-sm ml-2">({quote.customer.company_name})</span>
              )}
            </div>
          ...
          <div className="border-t border-slate-200 dark:border-zinc-700 pt-4 flex flex-col items-end space-y-2 text-sm">
            <div className="flex justify-between w-64 text-slate-600 dark:text-zinc-400">
              <span>Subtotal:</span>
              <span className="font-medium text-slate-900 dark:text-zinc-100">{formatCurrency(quote.subtotal_cents)}</span>
            </div>
            ...
            <div className="flex justify-between w-64 text-slate-600 dark:text-zinc-400">
              <span>Tax:</span>
              <span className="font-medium text-slate-900 dark:text-zinc-100">{formatCurrency(quote.tax_cents)}</span>
            </div>
          ...
          {(quote.notes || quote.terms) && (
            <div className="border-t border-slate-100 dark:border-zinc-800 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-500 dark:text-zinc-400">
              {quote.notes && (
                <div>
                  <span className="font-semibold text-slate-700 dark:text-zinc-300 block mb-1">Notes:</span>
                  <p>{quote.notes}</p>
                </div>
              )}
              {quote.terms && (
                <div>
                  <span className="font-semibold text-slate-700 dark:text-zinc-300 block mb-1">Terms & Conditions:</span>
                  <p>{quote.terms}</p>
                </div>
              )}
            </div>

  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link href="/customers" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-slate-800">
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              {customer.company_name && (
                <p className="text-sm font-medium text-slate-500">{customer.company_name}</p>
              )}
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-3 text-sm text-slate-600">
                <a
                  href={`tel:${customer.phone}`}
                  className="flex items-center text-blue-600 font-medium hover:underline min-h-[44px]"
                >
                ...
                {customer.email && (
                  <a
                    ...
                    className="flex items-center hover:underline min-h-[44px]"
                  >
                ...
                <a
                  ...
                  className="flex items-center text-slate-700 hover:text-blue-600 min-h-[44px]"
                >
                  <Navigation className="w-4 h-4 mr-1.5 text-slate-400" />
                  {customer.address_line1}, {customer.city}, {customer.state} {customer.postal_code}
                </a>
              </div>
            </div>
          ...
          {customer.notes && (
            <div className="mt-4 pt-4 border-t border-slate-100 text-xs text-slate-600 bg-amber-50/60 p-3 rounded-md border border-amber-100">
              <span className="font-semibold text-amber-900 block mb-0.5">Internal Access Notes:</span>
              {customer.notes}
            </div>
          )}
        </CardContent>
            ...
            {timeline.quotes.length === 0 ? (
              ...
              timeline.quotes.map((quote: any) => (
                <div key={quote.id} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between">
                  <div>
                    <Link href={`/quotes/${quote.id}`} className="font-semibold text-sm text-blue-600 hover:underline">
                      {quote.quote_number}
                    </Link>
                    <p className="text-xs text-slate-400 mt-0.5">{formatDate(quote.issue_date)}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-bold block">{formatCurrency(quote.total_cents)}</span>
        ...
        <Card>
          ...
          <CardContent className="p-4 divide-y divide-slate-100">
            {timeline.jobs.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">No jobs scheduled.</p>
            ) : (
              timeline.jobs.map((job: any) => (
                <div key={job.id} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between">
                  <div className="truncate mr-2">
                    <Link href={`/jobs/${job.id}`} className="font-semibold text-sm text-purple-600 hover:underline truncate block">
                      {job.job_number}
                    </Link>
            ...
            {timeline.invoices.length === 0 ? (
              ...
              timeline.invoices.map((inv: any) => (
                <div key={inv.id} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between">
                  <div>
                    <Link href={`/invoices/${inv.id}`} className="font-semibold text-sm text-emerald-600 hover:underline">
                      {inv.invoice_number}
                    </Link>
                    <p className="text-xs text-slate-400 mt-0.5">Due {formatDate(inv.due_date)}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-bold block">{formatCurrency(inv.total_cents)}</span>

        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-zinc-400">Official Invoice</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <a
              ...
              className="inline-flex items-center text-xs font-semibold text-slate-700 dark:text-zinc-300 bg-slate-100 dark:bg-zinc-800 px-3 py-2 rounded-md hover:bg-slate-200 dark:hover:bg-zinc-700 min-h-[44px]"
            >
          ...
          <CardHeader className="p-5 sm:p-8 border-b border-slate-100 dark:border-zinc-800 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h2 className="text-2xl font-black text-slate-900 dark:text-zinc-100">{invoice.invoice_number}</h2>
            ...
            {customer && (
              <div className="text-xs text-slate-600 dark:text-zinc-300 bg-slate-50 dark:bg-zinc-800/60 p-3 rounded-lg border border-slate-100 dark:border-zinc-700/80">
                <span className="font-semibold text-slate-900 dark:text-zinc-100 block mb-0.5">Billed To:</span>
          ...
          <CardContent className="p-5 sm:p-8 space-y-6">
            ...
            <div className="border-t border-slate-200 dark:border-zinc-800 pt-4 space-y-1.5 text-sm">
              ...
              {invoice.amount_paid_cents > 0 && (
                <div className="flex justify-between text-emerald-600 dark:text-emerald-400 text-sm font-semibold">
              ...
              <div className="flex justify-between text-base font-bold text-slate-900 dark:text-zinc-100 pt-2 border-t border-slate-100 dark:border-zinc-800">
                ...
                <span className={`text-xl font-black ${isPaid ? 'text-emerald-600 dark:text-emerald-400' : 'text-blue-600 dark:text-blue-400'}`}>
                  {formatCurrency(invoice.balance_due_cents, org?.currency)}
                </span>
              </div>
            </div>
            ...
            {invoice.terms && (
              <div className="text-xs text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-800/60 p-3 rounded-lg">
                <span className="font-semibold text-slate-700 dark:text-zinc-300 block mb-1">Payment Instructions:</span>
                {invoice.terms}
              </div>
            )}
          </CardContent>
